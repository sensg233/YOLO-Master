...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz: tuple[int, int, int, int] = (1, 3, 640, 640)) -> None:
        ...
        model_format = getattr(self, "format", None)
        if model_format is None:
            legacy_formats = {
                "pt": getattr(self, "pt", False),
                "torchscript": getattr(self, "jit", False),
                "onnx": getattr(self, "onnx", False),
                "engine": getattr(self, "engine", False),
                "saved_model": getattr(self, "saved_model", False),
                "pb": getattr(self, "pb", False),
                "triton": getattr(self, "triton", False),
                "nn_module": getattr(self, "nn_module", False),
            }
            model_format = next((name for name, enabled in legacy_formats.items() if enabled), None)
        if not getattr(self, "end2end", False):
            import torchvision  # noqa (import here triggers torchvision NMS use in nms.py)
        if model_format in {"pt", "torchscript", "onnx", "engine", "saved_model", "pb", "triton", "nn_module"} and (
            self.device.type != "cpu" or model_format == "triton"
        ):
            im = torch.zeros(*imgsz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input
            for _ in range(2 if model_format == "torchscript" else 1):
